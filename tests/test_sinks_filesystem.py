...
def test_filesystem_sink_writes_file(tmp_path):
    ...
    written = list((tmp_path / "acme-pipeline").iterdir())
    assert len(written) == 1
    assert written[0].read_bytes() == b"ISA*00*..."
    assert "873" in written[0].name


def test_filesystem_sink_creates_partner_subdirectory(tmp_path):
    sink = FilesystemSink(base_dir=str(tmp_path))
    asyncio.run(sink.deliver(_message(partner_name="other-partner")))
    assert (tmp_path / "other-partner").is_dir()
